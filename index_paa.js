...
var rate2 = require('./rate2');
var rootScope = rootScopeModule.rootScope;
rootScope.RATE = rate2.rate;

var sourceMappingCode = require('./sourceMappingCode');
var mappingCode = {};

const preparation = require('./alterPreparation');
...
app.post('/getPremi', function (req, res){
	...
	writeStreamBody = fs.createWriteStream("./log/" + tmpReq.policyNumber + '_request.json');
	writeStreamBody.write(req.body.param);
	
	writeStreamBodyA = fs.createWriteStream("./log/" + tmpReq.policyNumber + '_request_afterAlterPrep.json');
...
function reMappingCode(param, coverageCdList){	
	...
	var listMappingCode = sourceMappingCode.MapCode;

	mappingCode = {}
	listMappingCode.forEach((code, index) => mappingCode[code.childCode] = code.parentCode);
...
function writeToLogFile(log){
			...
			if(tempListRateCd){
				for(var j = 0; j < tempListRateCd.length; j++){
					...
					if(pphMax){
						...
						}else if(itemSelected.code.match(/H1Z1.*/)){
		...
		function checkingIfButtonHitung(buttonType, tmpFormula, formula, mapResultFormula, mapOutputCoverage, result, mapOutputFundAltLimited, 
			...
			if('hitung' === buttonType){
				...
				else if('HSPLAN' === formula.formulaTypeCd.toUpperCase()){
		...
		function checkCovTermPpayorAndCovTermWaiver(tmpFormula, itemSelected, mapOutputCoverage, result){
			...
			for(var i = 0; i < itemSelected.itemInput.length; i++){
				if(itemSelected.itemInput[i].code.indexOf('PLANOPTION') != -1){
					tmpCount = i;
					break;
				}
			}
...
//------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		function setStringFormulaForFormulaBasicByFormulaElementTypeIsFormulaFund(fe, stringFormula, stringFormulaAlt,
			...
			if(fe.value.toUpperCase() === 'TOTALCVLOWLASTYEAR' && paramMap.year > 1){
		...
		function predefinedCalculation(tempData, newManfaatList, mapProperties, param, mapOutputCoveragePrecalculated, i, isPphAlt, flagProcess){
			...
			if(tempData !== undefined){
				ITEM_PREDEFINED = rootScope.COVERAGE_PAA[tempData.code];
				var itemSelected = tempData;
				if(ITEM_PREDEFINED != undefined){
					var predefined;
					if(flagProcess == 'flagHitung'){
						predefined = ITEM_PREDEFINED.FORMULA.filter( function(item){return (item.category == itemCategory || item.category == undefined);} );
					}
					else{
						predefined = ITEM_PREDEFINED.FORMULA.filter( function(item){return ((item.category == itemCategory || item.category == undefined) && item.groupSequence != null);} );
					}
			...
			for(var i = 0; i < tempManfaatList.length; i++){
				ITEM_PREDEFINED = rootScope.COVERAGE_PAA[tempManfaatList[i].code];
			...
			for(var y = 0; y < listItemPredefined.length; y++){
			  ...
			  for(var j = 0; j < itemInputList.length; j++){
				...
				if(obj.type === 'COVERAGE'){
					...
					mapProperties['CUSTAGE'+'0'+(obj.itemSelected.tertanggungKey-1)] = obj.itemSelected.tertanggungAge;
				...
				}else if(obj.itemSelected.code == 'U10R' || obj.itemSelected.code == 'U10D' || obj.itemSelected.code == 'U11R'){
		...
		function processOnPreparedParameter(newManfaatList, mapProperties, manfaatListCodeSelected, param, flagProcess,
			...
			for(var i = 0; i < newManfaatList.length; i++){
				...
				if(itemSelected.type === 'COVERAGE'){
					ITEM = rootScope.COVERAGE_PAA[itemSelected.code];
					// if(itemSelected.code == 'D1ER'){
					// 	var xxx = 0;
					// }
					if(flagProcess == 'flagHitung'){
						ITEM.FORMULA_BOTH = ITEM.FORMULA.filter( function(item){return (item.category == itemCategory || item.category == undefined);} );
					}
					else{
						ITEM.FORMULA_BOTH = ITEM.FORMULA.filter( function(item){return ((item.category == itemCategory || item.category == undefined) && item.groupSequence == null);} );
					}
					...
					ITEM = rootScope.FUND_PAA[itemSelected.code];
					var x = ITEM.FORMULA.filter( function(item){return (item.category == itemCategory || item.category == undefined);} );
				...
				for(var j = 0; j < itemInputList.length; j++){
					if(itemInputList[j].key === 'PDALLO'){
						...
						if(!(itemInputList[j].key === 'PDPREMI' && itemSelected.code == mapProperties["PREVIOUSRIDERCODE"])){
							mapProperties[itemInputList[j].key] = itemInputList[j].inputValue;
						}
					}
				...
				if(itemSelected.type === 'COVERAGE'){
					...
					var lavAllocationValue = ITEM.BONUSALLOCATION_VALUE == undefined ? undefined : ITEM.BONUSALLOCATION_VALUE[param.year];
					mapProperties['BONUSALLOVALUE'] = lavAllocationValue == null ? 0 : lavAllocationValue;	
					
					//LAV_ALLOCATION VALUE
					var lavToBavAllocationValue = ITEM.LAVTOBPAVALLOCATION_VALUE == undefined ? undefined : ITEM.LAVTOBPAVALLOCATION_VALUE[param.year];
		...
		function processIlustration(param, DIFFLVPREMI){
			...
			if(res != undefined){
				isPPHC = false;
					if(res.status == '0'){
							...
							}else if(param.mainCoverage == 'T1PR' || param.mainCoverage == 'T1QR' || param.mainCoverage == 'T1RR' || param.mainCoverage == 'T1SR'){
		 ...
		 function getUnappliedPremium(param, flagProcess, DIFFLVPREMI){
		 	//kosoingin log
		 	rootScope.log = [];
		 	rootScope.log_pph_alt = [];
		 	rootScope.log_pph_client = [];
			mapChargeRider = {};
			param.year = 1;
			tmpCurr = param.currCd;
			 
			if(param.flagSaIncrease == 'N'){
				param.flagSaIncrease = 0;
			}
			else{
				param.flagSaIncrease = 1;
			}
			//set param.process
		...
		function preparedParameter(type, param, paramMapOutputFund, paramMapOutputFundAlt, flagProcess, pphProcess,DIFFLVPREMI){
			...
			if(type === 'hitung'){
				...
				mapResult.rule = getRuleValidation(param.mainCoverage, mapOutputCoverage, coverageList.concat(coverageGroupList), mymanfaatlist, param.process);
			}
		...
		function getResultFormula(itemSelected, ITEM, map, mapResult, mapFundPerYear, mapOutputCoverage, mapOutputCoverageAlt, mapOutputFund, mapOutputFundAlt, paramMap, flag, buttonType, DIFFLVPREMI, bufferArr, pdAlloArr, latestManfaat){
			...
			for(var j = 0; j < tempMapFormulaList.length; j++){
				...
				var formula = rootScope.FORMULA_PAA[tmpFormula.formulaCd];
				if(formula){
						var isProcess = false;
						...
						for(var k = 0; k < tempFormulaElementList.length; k++){
							...
							if(fe.type.toLowerCase().trim() === "coverage"
								...
								if(fe.value.toUpperCase() === 'CUSTINCOME'){
									...
									stringFormula += map[fe.value] && map[fe.value].toString().trim() != '' ? map[fe.value] : '0.0';
									stringFormulaAlt += map[fe.value] && map[fe.value].toString().trim() != '' ? map[fe.value] : '0.0';
								}
							...
							result = checkCovTermPpayorAndCovTermWaiver(tmpFormula, itemSelected, mapOutputCoverage, result);
								  ...
								  if(isPIA){
									  if(tmpFormula.output == 'PIAGIOIDR' && result == '1'){
										  mapResult['isGio'] = '1';					  				
									  }else if(tmpFormula.output == 'PIAUSDGIO' && result == '1'){
										  mapResult['isGio'] = '1';					  				
							...
							if(tmpFormula.output){
								if('COVERAGE' === tmpFormula.itemType.toUpperCase()){
									...
									if(value){
										...
										}else if(tmpFormula.output == 'PDSACHARGE'){
											mapOutputCoverage[tmpFormula.output] = result;
										}else{
									...
									if('RIDERPREMIUM' == formula.formulaTypeCd){
										if(map["PREVIOUSRIDERCODE"] == itemSelected.code){
								...
								if(fe.type.toLowerCase().trim() === "coverage"
								  ...
								  stringFormulaAlt += itemSelected.loadMap[fe.value] ? itemSelected.loadMap[fe.value] : '0.0';
									...
									if(fe.value.toUpperCase() === 'MAXLVPREMI'){
								  	...
								  	if((fe.value.toUpperCase() == 'CVTOTALLOWLASTYEAR' || fe.value.toUpperCase() == 'CVTOTALMEDLASTYEAR' || fe.value.toUpperCase() == 'CVTOTALHIGHLASTYEAR'
		 ...
		 function generateOutputROP(param, mapOutputMain){
			...
			for(var year in mapOutput){
				...
				var objGabFund = {
					...
					customerAge : (mapItemCd.ageCustomer).toString(),
					premiClient : parseInt(year) <= parseInt(param.rencanaPembayaran) ? ((param.manfaat.totalPremi)/divider).toString() : '',
					totalPremium : mapFundNLG['FT_TOTAL_PREMIUM_ROP'].toString(),
					standardPremium : mapFundNLG['FT_PREMIUM_ROP'].toString(),
					surrenderValue : mapFundNLG['FT_SV_ROP'].toString(),
					deathBenefit : mapFundNLG['FT_DEATH_BENEFIT_ROP'].toString(),
					deathBenefitUponAccident : mapFundNLG['FT_DEATH_BENEFIT_ACCIDENT_ROP'].toString(),
					angioplastyBenefit : mapFundNLG['FT_ANGIOPLASTY_BENEFIT_ROP'].toString(),
					maturity : mapFundNLG['FT_ANGIOPLASTY_BENEFIT_ROP'].toString(),
					accumulationPremium : mapFundNLG['TOTALCVLOW'].toString()
				};
		...
		function getRuleValidation(mainCoverage, mapOutputCoverage,itemList, manfaatList, isAlterProcess){
			  ...
			  for(var i = 0; i < itemList.length; i++){
				  ...
				  if(itemList[i].itemType === 'COVERAGE_GROUP' ||
					 ...
					 itemList[i].itemType === 'Coverage_Group'){
					  ...
					  itemRuleList = itemRuleList.filter( function(item){return (item.category == itemCategory || item.category == undefined);} );
					  isCoverageGroup = true;
				  }else{
					  /*if(itemCategory == 'Alter' && !itemList[i].isNeedToBeCalculated){
						continue;
					  }*/
					  itemRuleList = rootScope.COVERAGE_PAA[itemList[i].itemCd].RULE;
					  itemRuleList = itemRuleList.filter( function(item){return (item.category == itemCategory || item.category == undefined);} );
				  ...
				  for(var j = 0; j < itemRuleList.length; j++){
					  ...
					  if(exec){
						  ...
						  var tmpRule = rootScope.RULE_PAA[itemRule.ruleCd+'|'+cov];
						  if(tmpRule){
		...
		function formulaRule(itemCd, formulaCd, map, mapOutputCoverage, mapOutputFund){
			var frml = rootScope.FORMULA_PAA[formulaCd];
			 var stringFormula = '';
			 var stringFormulaOri = '';
			var itemType = '';
			 var result = 0;
			...
			if(frml){
				 ...
				 if(itemCd === 'D1DR ' && formulaCd === 'FRML_MAXSA_TPD_02ALT'){
					var xxx = 0;
				 }
		...
		function formulaPrecalculate(obj, param, map, mapOutputCoverage){
			var frml = rootScope.FORMULA_PAA[obj.formulaCd];
			 var stringFormula = '';
			 var stringFormulaAlt = '';
			 var stringFormulaOri = '';
			var itemType = '';
			...
			if(frml){
				 ...
				 for(var i= 0; i < forElmList.length; i++){
					...
					if(fe.type.toLowerCase().trim() === "customer"
					  ...
					  }else if(fe.value.toUpperCase() == 'TOTALPREMIUMWITHACCPREMIUMLBDB' && obj.formulaCd == 'FRML_TOT_PREMIUM_LB_CLIENT_PRECALCULATED'){
					  ...
					  }else if(fe.value.toUpperCase() == 'TOTALPREMIUMWITHACCPREMIUMLBDB' && obj.formulaCd == 'FRML_TOT_PREMIUM_LB_ALT_PRECALCULATED'){
				 ...
				 if(isValidExpression(stringFormula)){
					...
					if(obj.output == 'TOTALPREMIUMWITHACCPREMIUMLBDB' && obj.formulaCd == 'FRML_TOT_PREMIUM_LB_CLIENT_PRECALCULATED'){
						param[obj.output+'CLIENT'] = result;
					}

					if(obj.output == 'TOTALPREMIUMWITHACCPREMIUMLBDB' && obj.formulaCd == 'FRML_TOT_PREMIUM_LB_ALT_PRECALCULATED'){
		...
		function formulaPrecalculateALT(obj, param, map, mapOutputCoverage){
			...
			if(frml){
				 ...
				 if(isValidExpression(stringFormula)){
					...
					if(obj.output == 'DIFFLVPREMI' && obj.itemSelected.isPPH == 'O' && param.year == '1'){
						param['DIFFLVPREMI_'+obj.itemSelected.isPPH] = result;
					}
		...
		function sortingRiderTopupMain(tempManfaatList, age){
				  ...
				  for(var j = 0; j < tmpManfaat.custList.length; j++){
					 ...
					 if(tmpCustList.customerId != undefined){
					  ...
					  if(tmpManfaat.code.match(/H1X.*/) || tmpManfaat.code.match(/H1Y.*/)){
						...
						manfaat.isPPH = 'O';
						var coverageItemInput = rootScope.COVERAGE_PAA[tmpManfaat.code].ITEM_INPUT;
						...
						while(true){
							...
							var splitValue2;
							for(var kk=0; kk<splitValue.length; kk++){
								if(splitValue[kk].indexOf(mapManfaatItemInput[itemInput.key]) != -1){
			...
			for(var i = 0; i < manfaatList.length; i++){
				 var coverage = rootScope.COVERAGE_PAA[manfaatList[i].code];
				 if(coverage.type.toLowerCase().trim() === 'rider'){
					...
					} else if (manfaatList[i].code.toUpperCase().trim() == 'H1Z1'){
						//tambah flag sorting
						var tempPPH = JSON.stringify(manfaatList[i]);
						var jj;
						for(jj = 0; jj < manfaatList[i].itemInput.length; jj++){
		...
		function preparedParameterClient(type, param, mapOutputFund, mapOutputFundAlt, flagProcess, DIFFLVPREMI, bufferArr, pdAlloArr){
			...
			for(var i = 0;i < newFundList.length; i++){
				...
				var _fundDetail = rootScope.FUND_PAA[_fund.code];
		...
		function getRuleValidationFundAvailable(mainCoverage, mapOutputCoverage,itemList, manfaatList){
			  ...
			  for(var i = 0; i < itemList.length; i++){
				  ...
				  itemRuleList = rootScope.COVERAGE_PAA[mainCoverage].RULE;
				  ...
				  for(var j = 0; j < itemRuleList.length; j++){
					  ...
					  if(exec){
						  keyTertanggungAge = itemList[i].keyTertanggungAge;
						  var tmpRule = rootScope.RULE_PAA[itemRule.ruleCd+'|'+itemRule.itemCd];
